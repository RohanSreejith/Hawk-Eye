...
class CameraStreamer:
    ...
    def set_video(self, path: str):
        if self.video_path == path and self.cap is not None and self.cap.isOpened():
            return
        with _cv_lock:
    ...
    def _reader_worker(self):
        """
        Reads frames from disk at natural 30 FPS.
        Applies the latest known AI bounding boxes and HUD overlays in <1ms,
        encodes to JPEG, and updates self.latest_jpeg immediately.
        This guarantees silky-smooth 30 FPS playback with ZERO frame skipping.
        """
    ...
    def _draw_hud(self, frame, mode, is_collision_hazard, closest_pair,
        ...
        if mode == "fire" and self.camera_id in ("HAWK_B", "b"):
            ...
            bracket_len = 22
            corner_col = (0, 220, 255)
            # Top-left
            cv2.line(canvas, (fx1, fy1), (fx1 + bracket_len, fy1), corner_col, 3)
            cv2.line(canvas, (fx1, fy1), (fx1, fy1 + bracket_len), corner_col, 3)
            # Top-right
            cv2.line(canvas, (fx2, fy1), (fx2 - bracket_len, fy1), corner_col, 3)
            cv2.line(canvas, (fx2, fy1), (fx2, fy1 + bracket_len), corner_col, 3)
            # Bottom-left
            cv2.line(canvas, (fx1, fy2), (fx1 + bracket_len, fy2), corner_col, 3)
            cv2.line(canvas, (fx1, fy2), (fx1, fy2 - bracket_len), corner_col, 3)
            # Bottom-right
            cv2.line(canvas, (fx2, fy2), (fx2 - bracket_len, fy2), corner_col, 3)
            cv2.line(canvas, (fx2, fy2), (fx2, fy2 - bracket_len), corner_col, 3)
...
class CameraManager:
    ...
    def set_scenario_mode(self, mode: str):
        """Sets scenario mode across both HAWK camera streamers ('baseline', 'anticipate', 'conflict', 'fire')."""
        self.hawk_streamer_a.scenario_mode = mode
        self.hawk_streamer_b.scenario_mode = mode
        print(f"[CAMERA MANAGER] HAWK Streamers mode set to: {mode}")
